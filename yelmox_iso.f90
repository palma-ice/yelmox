...
!                       dim1="xc",dim2="yc",dim3="time",start=[1,1,n],ncid=ncid)
    ...
    subroutine modify_smb(smb,grd,time)
        ...
        dsmb_0kyr = 0.0_prec 
        call scale_cf_gaussian(dsmb_0kyr,-1.5,x0= 600.0, y0=-1800.0,sigma=50.0,xx=grd%x*1e-3,yy=grd%y*1e-3)
        ...
        dsmb_8kyr = 0.0_prec 
        call scale_cf_gaussian(dsmb_8kyr,-0.5,x0= 600.0, y0=-1800.0,sigma=50.0,xx=grd%x*1e-3,yy=grd%y*1e-3)
        call scale_cf_gaussian(dsmb_8kyr,-0.5,x0= 600.0, y0=-1900.0,sigma=50.0,xx=grd%x*1e-3,yy=grd%y*1e-3)
        call scale_cf_gaussian(dsmb_8kyr,-0.5,x0= 600.0, y0=-2000.0,sigma=50.0,xx=grd%x*1e-3,yy=grd%y*1e-3)
        call scale_cf_gaussian(dsmb_8kyr,-0.5,x0= 600.0, y0=-2100.0,sigma=50.0,xx=grd%x*1e-3,yy=grd%y*1e-3)
        
        call scale_cf_gaussian(dsmb_8kyr, 0.2,x0=-300.0, y0=-1200.0,sigma=200.0,xx=grd%x*1e-3,yy=grd%y*1e-3)
        call scale_cf_gaussian(dsmb_8kyr, 0.2,x0= 300.0, y0=-1100.0,sigma=200.0,xx=grd%x*1e-3,yy=grd%y*1e-3)
