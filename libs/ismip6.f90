...
contains
    ...
    subroutine nahosmip_ant_forcing_init(ism,filename,domain,grid_name,gcm,scenario,experiment)
        ...
        ! Special case for control runs, use "CCSM4_RCP85"
        ! antoniojm: This is not being used. It is to read something. Ctrl sets atm = 0 (anomalies) and
        ! ocn = Jourdain (ref)
        if (trim(scenario_now) .eq. "ctrl" .or. trim(scenario_now) .eq. "ctrl0") then
            ! Assign specific gcm+scenario for control runs

            ism%gcm        =  "HadGEM3"
            ism%scenario   =  "GC3-1MM"
            ism%experiment =  "HadGEM3_GC3-1MM"
    ...
    subroutine nahosmip_ant_forcing_update(ism,time,use_ref_atm,use_ref_ocn)
        ...
        if (trim(ism%ctrl_run_type) .eq. "ctrl0") then 
            ...
            ! antoniojm: atmospheric fields as in ctrl and spinup (RACMO+reanalysis)
            ! Set atm fields to reference values (ie, zero anomaly) 
            ism%ts  = ism%ts_ref 
            ism%pr  = ism%pr_ref 
            ism%smb = ism%smb_ref 
        ...
        else ! time .gt. 2300 !antoniojm: not relevant now
            ! Take the average of the last 10 years of projection forcing data

            call varslice_update(ism%ts_proj, [2015.0_wp,2015.0_wp],method="range_mean")
            call varslice_update(ism%pr_proj, [2015.0_wp,2015.0_wp],method="range_mean")
            call varslice_update(ism%smb_proj,[2015.0_wp,2015.0_wp],method="range_mean")
        ...
        ! antoniojm
        else if (time .ge. 2015 .and. time .le. 2149) then
            ! Projection period 1 
            ...
            call varslice_update(ism%to_proj,[2140.0_wp,2149.0_wp],method="range_mean")
            call varslice_update(ism%so_proj,[2015.0_wp,2015.0_wp],method="range_mean")
            call varslice_update(ism%tf_proj,[2140.0_wp,2149.0_wp],method="range_mean")
    ...
    subroutine ismip6_ant_forcing_update(ism,time,use_ref_atm,use_ref_ocn)
        ...
        if (trim(ism%ctrl_run_type) .eq. "ctrl0") then 
            ...
            !antoniojm: for same forcing as in ctrl
