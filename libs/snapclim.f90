...
contains 
    ...
    subroutine snapclim_init(snp,filename,group,domain,grid_name,nx,ny,basins)
        ...
        ! Load parameters 
        call snapclim_par_load(snp%par,snp%hybrid,filename,group=group)
        snp%par%nx = nx 
        snp%par%ny = ny 

        ! If using recon method load parameters 
        if (trim(snp%par%atm_type) .eq. "recon") then 
            call recon_par_load(snp%recon,filename,domain,grid_name,group=trim(group)//"_recon")
        end if 
        ...
        call snapshot_par_load(snp%clim0%par,filename,trim(group)//"_clim0",domain,grid_name,init=.TRUE.)
        ...
        if (load_atm1 .or. load_ocn1) then
            ...
            call snapshot_par_load(snp%clim1%par,filename,trim(group)//"_clim1",domain,grid_name,init=.TRUE.)                
        ...
        if (load_atm2 .or. load_ocn2) then
            ...
            call snapshot_par_load(snp%clim2%par,filename,trim(group)//"_clim2",domain,grid_name,init=.TRUE.)                
        ...
        if (load_atm3 .or. load_ocn3) then
            ...
            call snapshot_par_load(snp%clim3%par,filename,trim(group)//"_clim3",domain,grid_name,init=.TRUE.)
...
!         temp_now = temp0 + aa*( (1.0-bb)*temp2 + bb*temp3 - temp1)
    ...
    subroutine snapclim_par_load(par,hpar,filename,init,group)
        ...
        call nml_read(filename,group,"atm_type",           par%atm_type,       init=init_pars)
        call nml_read(filename,group,"ocn_type",           par%ocn_type,       init=init_pars)
        call nml_read(filename,group,"fname_at",           par%fname_at,       init=init_pars)
        call nml_read(filename,group,"fname_ao",           par%fname_ao,       init=init_pars)
        call nml_read(filename,group,"fname_ap",           par%fname_ap,       init=init_pars)
        call nml_read(filename,group,"fname_as",           par%fname_as,       init=init_pars)        
        call nml_read(filename,group,"fname_bt",           par%fname_bt,       init=init_pars)
        call nml_read(filename,group,"fname_bo",           par%fname_bo,       init=init_pars)
        call nml_read(filename,group,"fname_bp",           par%fname_bp,       init=init_pars)
        call nml_read(filename,group,"fname_bs",           par%fname_bs,       init=init_pars)
        call nml_read(filename,group,"lapse",              par%lapse,          init=init_pars)
        call nml_read(filename,group,"dTa_const",          par%dTa_const,      init=init_pars)
        call nml_read(filename,group,"dTo_const",          par%dTo_const,      init=init_pars)
        call nml_read(filename,group,"dSo_const",          par%dSo_const,      init=init_pars)
        call nml_read(filename,group,"f_to",               par%f_to,           init=init_pars)
        call nml_read(filename,group,"f_p",                par%f_p,            init=init_pars)
        !call nml_read(filename,group,"f_p_ne",             par%f_p_ne,         init=init_pars)
        call nml_read(filename,group,"f_stdev",            par%f_stdev,        init=init_pars)
        
        call nml_read(filename,trim(group)//"_hybrid","hybrid_path", hpar%hybrid_path,  init=init_pars)
        call nml_read(filename,trim(group)//"_hybrid","f_eem",       hpar%f_eem,        init=init_pars)
        call nml_read(filename,trim(group)//"_hybrid","f_glac",      hpar%f_glac,       init=init_pars)
        call nml_read(filename,trim(group)//"_hybrid","f_hol",       hpar%f_hol,        init=init_pars)
        call nml_read(filename,trim(group)//"_hybrid","f_seas",      hpar%f_seas,       init=init_pars)
        call nml_read(filename,trim(group)//"_hybrid","f_to",        hpar%f_to,         init=init_pars)
    ...
    subroutine recon_par_load(par,filename,domain,grid_name,init,group)
        ...
        call nml_read(filename,trim(group),"clim_path",    par%clim_path,    init=init_pars)
        call nml_read(filename,trim(group),"clim_names",   par%clim_names,   init=init_pars)
        call nml_read(filename,trim(group),"clim_monthly", par%clim_monthly, init=init_pars)

        call nml_read(filename,trim(group),"ocn_path",     par%ocn_path,     init=init_pars)
        call nml_read(filename,trim(group),"ocn_names",    par%ocn_names,    init=init_pars)
        call nml_read(filename,trim(group),"ocn_monthly",  par%ocn_monthly,  init=init_pars)
